...
begin
  ...
  v_service:=public.emitir_gift_card_v1(jsonb_build_object('request_id','28000000-0000-4000-8000-000000000001','tipo','SERVICIO','service_code','SVC_001','comprador','QA comprador','beneficiario','QA servicio','whatsapp_beneficiario','+51987654321','sede','Miraflores','metodo_pago','YAPE','numero_operacion','QA-VENTA-S','monto_recibido',70,'responsable','QA'));
  ...
  v_hold:=public.preparar_ficha_cita_gift_card_v1(v_base||jsonb_build_object('request_id','28000000-0000-4000-8000-000000000002','giftcard_id',v_gc_service,'movimiento_id','MOV-QA-028-S1','reserva_id','RES-QA-028-S1','pago_id','PAY-QA-028-S1','token',repeat('S',43),'servicios',jsonb_build_array(jsonb_build_object('codigo','SVC_001'))));
  ...
  begin perform public.preparar_ficha_cita_gift_card_v1(v_base||jsonb_build_object('request_id','28000000-0000-4000-8000-000000000003','giftcard_id',v_gc_service,'movimiento_id','MOV-QA-028-S2','reserva_id','RES-QA-028-S2','pago_id','PAY-QA-028-S2','token',repeat('T',43),'servicios',jsonb_build_array(jsonb_build_object('codigo','SVC_001')))); raise exception 'CASE_02'; exception when sqlstate '22023' then null; end;

  -- CASE_14 replay idéntico / CASE_15 payload distinto.
  v_replay:=public.preparar_ficha_cita_gift_card_v1(v_base||jsonb_build_object('request_id','28000000-0000-4000-8000-000000000002','giftcard_id',v_gc_service,'movimiento_id','MOV-IGNORED','reserva_id','RES-IGNORED','pago_id','PAY-IGNORED','token',repeat('X',43),'servicios',jsonb_build_array(jsonb_build_object('codigo','SVC_001'))));
  ...
  v_hold:=public.preparar_ficha_cita_gift_card_v1(v_base||jsonb_build_object('request_id','28000000-0000-4000-8000-000000000005','giftcard_id',v_gc_service,'movimiento_id','MOV-QA-028-S3','reserva_id','RES-QA-028-S3','pago_id','PAY-QA-028-S3','token',repeat('U',43),'servicios',jsonb_build_array(jsonb_build_object('codigo','SVC_001'))));
  ...
  v_hold:=public.preparar_ficha_cita_gift_card_v1(v_base||jsonb_build_object('request_id','28000000-0000-4000-8000-000000000009','giftcard_id',v_gc_amount,'movimiento_id','MOV-QA-028-M1','reserva_id','RES-QA-028-M1','pago_id','PAY-QA-028-M1','token',repeat('M',43),'servicios',jsonb_build_array(jsonb_build_object('codigo','SVC_001'))));
  v_second:=public.preparar_ficha_cita_gift_card_v1((v_base||jsonb_build_object('cliente','QA monto 2','whatsapp_e164','+51987654322'))||jsonb_build_object('request_id','28000000-0000-4000-8000-000000000010','giftcard_id',v_gc_amount,'movimiento_id','MOV-QA-028-M2','reserva_id','RES-QA-028-M2','pago_id','PAY-QA-028-M2','token',repeat('N',43),'servicios',jsonb_build_array(jsonb_build_object('codigo','SVC_001'))));
