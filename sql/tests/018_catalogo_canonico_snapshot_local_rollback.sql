...
declare
  ...
  bad jsonb; code text;
begin
  select jsonb_agg(jsonb_build_object('release_id','catalog-v1-web-4104385','service_code','SVC_'||lpad(n::text,3,'0'),'slug','qa-'||n,'name_es','QA '||n,'name_en',null,'included_es',null,'included_en',null,'category',case when n in (8,9) then 'HOME' when n<=7 or n between 10 and 25 then 'INDIVIDUAL' when n between 26 and 39 then 'PACKAGE_TWO' when n<=43 then 'PROGRAM' when n<=46 then 'BEAUTY' else 'FACIAL' end,'commercial_group',null,'modality','QA','duration_min',60,'people_rule_status','CONFIRMED','people_min',case when n between 26 and 39 then 2 else 1 end,'people_max',case when n between 26 and 39 or n in (8,9) then 2 else 1 end,'selection_rule',case when n in (8,9) then 'HOME_FLOW' else 'QA' end,'reservation_behavior',case when n in (8,9) then 'HOME_APPOINTMENT' else 'QA' end,'component_eligible',null,'component_eligibility_status','PENDING_REVIEW','active',true,'price_pen',100,'previous_price_pen',null,'price_version','catalog-v1-web-4104385','valid_from','2026-09-01T00:00:00+00:00','valid_to',null) order by n) into s from generate_series(1,50) n;
