...
declare
 ...
 v_item jsonb; v_persona int; v_componente int := 0;
 v_fingerprint text:=md5((p_payload-'request_id'-'cliente_id'-'movimiento_id'-'reserva_id'-'pago_id'-'token')::text);
begin
 ...
 with personas as (
   select ordinality::int as persona, value as datos
   from jsonb_array_elements(v_componentes) with ordinality
 ), componentes as (
   select p.persona, c.ordinality::int as posicion, c.value as componente
   from personas p
   ...
   select c.persona, c.posicion, c.componente->>'tipo' as tipo,
   ...
   select persona, count(*) as cantidad_componentes,
 ...
 select coalesce(jsonb_agg(jsonb_build_object('persona', persona, 'componentes', normalizados) order by persona), '[]'::jsonb),
   ...
   count(*) = v_personas and bool_and(cantidad_componentes > 0 and valido)
 ...
 for v_persona, v_item in
   ...
   from jsonb_array_elements(v_componentes) as persona(value)
   cross join lateral jsonb_array_elements(persona.value->'componentes') as componente(value)
 loop
   ...
   values(v_res || '-C' || v_componente,v_mov,v_fecha,v_sede,v_persona,'Por asignar',btrim(v_item->>'nombre'),(v_item->>'duracion_min') || ' min',(v_item->>'precio')::numeric,nullif(btrim(p_payload->>'observacion'),''));
 end loop;
