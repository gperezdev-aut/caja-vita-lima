...
import { findGiftCardClients } from "@/lib/giftCardUx";
...
export function GiftCardsModule({
  ...
  const selectedClient = useMemo(() => clients.find((item) => item.id === buyerId), [buyerId, clients]);
  const buyerMatches = useMemo(() => findGiftCardClients(clients, buyer), [buyer, clients]);

  function changeBuyer(name: string) {
    setBuyer(name); setBuyerId(""); setBuyerResultsOpen(true);
  }
  function selectBuyer(client: Client) {
    setBuyer(client.name); setBuyerId(client.id); setBuyerPhone(client.whatsapp); setBuyerResultsOpen(false);
  }
  ...
  if (state.ok && state.giftcardId && state.code) {
    ...
    return <section className="panel giftCardSuccess" aria-live="polite"><span className="successMark">✓</span><p className="eyebrow">Gift Card emitida</p><h2>{state.code}</h2><p>Vigente hasta el {formatGiftCardDate(state.expiresAt || expiration)}.</p><div className="giftCardSuccessActions"><a className="primaryButton" href={`/api/gift-cards/${encodeURIComponent(state.giftcardId)}/download`}>Descargar Gift Card</a><Link className="ghostButton giftCardViewAction" href={detailPath}>Ver Gift Card</Link>{(beneficiaryPhone || buyerPhone) && <a className="ghostButton" href={share} target="_blank" rel="noreferrer">Compartir por WhatsApp</a>}<Link className="textButton" href="/gift-cards">Emitir otra</Link></div></section>;
  }
  ...
  return <>
    <form action={action} className="panel giftCardWizard" noValidate>
      ...
      {step === 0 && <section className="giftCardStep"><p className="stepKicker">Paso 1 de 4</p><h2>Tipo y beneficiario</h2><div className="giftCardTypeGrid"><button type="button" className={type === "SERVICIO" ? "selected" : ""} onClick={() => chooseType("SERVICIO")}><strong>Por servicio</strong><small>Un servicio canónico, un solo uso.</small></button><button type="button" className={type === "MONTO" ? "selected" : ""} onClick={() => chooseType("MONTO")}><strong>Por monto</strong><small>Saldo monetario con usos parciales.</small></button></div><div className="giftCardPeopleGrid"><section className="giftCardPersonGroup"><h3>Comprador</h3><div className="giftCardBuyerField" onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setBuyerResultsOpen(false); }}><label htmlFor="gift-card-buyer">Nombre o WhatsApp</label><input id="gift-card-buyer" role="combobox" aria-autocomplete="list" aria-expanded={buyerResultsOpen} aria-controls="gift-card-buyer-results" value={buyer} onChange={(event) => changeBuyer(event.target.value)} onFocus={() => setBuyerResultsOpen(true)} autoComplete="off" placeholder="Escribe para buscar o ingresar manualmente"/>{buyerResultsOpen && <div className="giftCardBuyerResults" id="gift-card-buyer-results" role="listbox">{buyerMatches.results.length ? buyerMatches.results.map((client) => <button key={client.id} type="button" role="option" aria-selected={client.id === buyerId} onClick={() => selectBuyer(client)}><strong>{client.name}</strong><small>{client.whatsapp || "Sin WhatsApp"}</small></button>) : <p>No hay coincidencias. Puedes continuar con el nombre ingresado.</p>}{buyerMatches.total > buyerMatches.results.length && <small className="giftCardBuyerMore">Sigue escribiendo para acotar {buyerMatches.total} resultados.</small>}</div>}</div><label>WhatsApp comprador<input value={buyerPhone} onChange={(event) => setBuyerPhone(event.target.value)} inputMode="tel" placeholder="987 654 321"/></label>{selectedClient && <p className="fieldMessage">Cliente seleccionado · {selectedClient.whatsapp || "sin WhatsApp"}</p>}</section><section className="giftCardPersonGroup"><h3>Beneficiario</h3><label>Nombre<input value={beneficiary} onChange={(event) => setBeneficiary(event.target.value)} autoComplete="name"/></label><label>WhatsApp (opcional)<input value={beneficiaryPhone} onChange={(event) => setBeneficiaryPhone(event.target.value)} inputMode="tel" placeholder="Solo si deseas registrarlo"/></label><p className="giftCardFieldHint">El beneficiario se registra como texto; no se crea ni modifica un cliente.</p></section></div><label className="giftCardDedication">Dedicatoria (opcional)<textarea rows={2} value={message} maxLength={280} onChange={(event) => setMessage(event.target.value)}/></label></section>}
