...
export default async function GiftCardDetailPage({ params }: { params: Promise<{ giftcard_id: string }> }) {
  ...
  return <main className="appShell"><CajaSidebar session={session}/><section className="page giftCardDetailPage">
    ...
    <section className="digitalGiftCard giftCardTemplatePreview"><Image unoptimized src={`/api/gift-cards/${encodeURIComponent(giftcardId)}/download?preview=1`} alt={`Vista previa de la Gift Card ${code} para ${beneficiary}`} width={1645} height={1379}/></section>
