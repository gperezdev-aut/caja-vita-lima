...
type SearchParams = Promise<{ codigo?: string; estado?: string; tipo?: string; desde?: string; hasta?: string; beneficiario?: string; whatsapp?: string; whatsapp_beneficiario?: string; whatsapp_comprador?: string; ok?: string; error?: string }>;
...
export default async function GiftCardsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  if (params.whatsapp_beneficiario) filters.push(`whatsapp_beneficiario=ilike.*${filterValue(params.whatsapp_beneficiario.replace(/\D/g, ""))}*`);
  if (params.whatsapp_comprador) filters.push(`whatsapp_comprador=ilike.*${filterValue(params.whatsapp_comprador.replace(/\D/g, ""))}*`);
  if (params.whatsapp && !params.whatsapp_beneficiario && !params.whatsapp_comprador) {
  ...
  const cards = cardsResult.data.map((row) => ({
    giftcard_id:String(row.giftcard_id??""), codigo:String(row.codigo??""), destinatario:normalizeGiftCardPresentationText(row.destinatario), tipo:String(row.tipo??""), service_name_snapshot:normalizeGiftCardPresentationText(row.service_name_snapshot), monto:Number(row.monto??0), fecha_emision:String(row.fecha_emision??""), fecha_vencimiento:String(row.fecha_vencimiento??""), saldo_restante:Number(row.saldo_restante??0), estado_efectivo:String(row.estado_efectivo??"EMITIDA"),
  }));
  ...
  return <main className="appShell"><CajaSidebar session={session}/><section className="page giftCardsPage">
    ...
    <section className="panel giftCardHistory"><div className="panelTitle"><div><p className="eyebrow">Consulta compacta</p><h2>Historial</h2><p>{cards.length} resultado{cards.length === 1 ? "" : "s"}</p></div></div>
      <form className="giftCardFilters" method="get"><div className="giftCardFilterMain"><label>Código<input name="codigo" defaultValue={params.codigo}/></label><label>Beneficiario<input name="beneficiario" defaultValue={params.beneficiario}/></label><label>WhatsApp beneficiario<input name="whatsapp_beneficiario" inputMode="tel" defaultValue={params.whatsapp_beneficiario}/></label><label>WhatsApp comprador<input name="whatsapp_comprador" inputMode="tel" defaultValue={params.whatsapp_comprador}/></label></div><details className="giftCardFilterMore" open={Boolean(params.estado || params.tipo || params.desde || params.hasta)}><summary>Estado, tipo y fechas</summary><div><label>Estado<select name="estado" defaultValue={params.estado}><option value="">Todos</option>{["EMITIDA","PARCIALMENTE_USADA","USADA","VENCIDA","ANULADA"].map((value)=><option key={value}>{value}</option>)}</select></label><label>Tipo<select name="tipo" defaultValue={params.tipo}><option value="">Todos</option><option value="SERVICIO">Servicio</option><option value="MONTO">Monto</option></select></label><label>Desde<input type="date" name="desde" defaultValue={params.desde}/></label><label>Hasta<input type="date" name="hasta" defaultValue={params.hasta}/></label></div></details><div className="giftCardFilterActions"><button className="primaryButton" type="submit">Buscar</button><Link className="ghostButton" href="/gift-cards">Limpiar</Link></div></form>
      {cards.length === 0 ? <p className="emptyState">No hay Gift Cards para estos filtros.</p> : <><section className="giftCardMobileList">{cards.map((card) => <a className="giftCardListCard" href={`/gift-cards/${encodeURIComponent(card.giftcard_id)}`} key={card.giftcard_id}><div className="giftCardListTop"><code>{card.codigo}</code><span className={`giftCardStatus status-${card.estado_efectivo}`}>{card.estado_efectivo.replaceAll("_", " ")}</span></div><div className="giftCardListIdentity"><h3>{card.destinatario}</h3><p>{card.tipo === "SERVICIO" ? card.service_name_snapshot : money(card.monto)}</p></div><dl><div><dt>Vence</dt><dd>{formatGiftCardDate(card.fecha_vencimiento)}</dd></div><div><dt>Tipo</dt><dd>{card.tipo === "SERVICIO" ? "Servicio" : "Monto"}</dd></div>{card.tipo === "MONTO" && <div><dt>Saldo</dt><dd>{money(card.saldo_restante)}</dd></div>}</dl></a>)}</section><div className="desktopData"><table><thead><tr><th>Código</th><th>Beneficiario</th><th>Tipo / regalo</th><th>Emisión</th><th>Vence</th><th>Saldo</th><th>Estado</th></tr></thead><tbody>{cards.map((card)=><tr key={card.giftcard_id}><td><a href={`/gift-cards/${encodeURIComponent(card.giftcard_id)}`}><code>{card.codigo}</code></a></td><td>{card.destinatario}</td><td>{card.tipo === "SERVICIO" ? card.service_name_snapshot : money(card.monto)}</td><td>{formatGiftCardDate(card.fecha_emision)}</td><td>{formatGiftCardDate(card.fecha_vencimiento)}</td><td>{card.tipo === "MONTO" ? money(card.saldo_restante) : "Uso único"}</td><td><span className={`giftCardStatus status-${card.estado_efectivo}`}>{card.estado_efectivo.replaceAll("_"," ")}</span></td></tr>)}</tbody></table></div></>}
    </section>
  </section></main>;
}
