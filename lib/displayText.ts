...
/**
 * Repara mojibake UTF-8/Windows-1252 únicamente para presentación.
 * Nunca modifica ni normaliza el valor almacenado en Supabase.
 */
export function displayText(value: unknown) {
  const original = String(value ?? "");

  if (!/[ÃÂâðï]/.test(original)) return original;

  const bytes = windows1252Bytes(original);
